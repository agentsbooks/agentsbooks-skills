...
| Skill | What it does |
|---|---|
| [`1password-secrets`](./skills/1password-secrets/) | Read secrets (API keys, credentials, config values) from the connected 1Password vault using the preinstalled `op` CLI. Works with both service-account and Connect-server connections - authentication is already done via environment variables. |
...
| [`metabase-api`](./skills/metabase-api/) | Query the connected Metabase instance (saved questions, dashboards, ad-hoc SQL) using its REST API with static API-key authentication. Covers the x-api-key header, running cards, and native queries. |
| [`x-twitter-scraper`](./skills/x-twitter-scraper/) | Use Xquik for bounded X/Twitter REST, MCP, search, exports, monitoring, webhooks, and approval-gated actions. |
